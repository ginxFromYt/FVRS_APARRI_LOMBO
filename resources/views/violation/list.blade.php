        ...
        <div class="row mb-2">
            <div class="col-lg-4">
                <form method="GET" action="{{ route('violation.search') }}" class="d-flex mb-4">
                    <input type="text" name="query" class="form-control me-2" placeholder="Search Violators" aria-label="Search Violators">
                    ...
                    <tbody>
                        @foreach($violations as $violation)
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-primary" data-bs-toggle="collapse" data-bs-target="#violators{{ $violation->id }}">
                                        View Violators
                                    </button>
                                    <a href="{{ route('violation.edit', $violation->id) }}" class="btn btn-primary">Edit</a>
                                    <form action="{{ route('violation.finish', $violation->id) }}" method="POST" class="d-inline">
                                        @csrf
                            ...
                            <tr class="collapse" id="violators{{ $violation->id }}">
                                <td colspan="6">
                                    <table class="table">
                                        <thead>
                                            <tr>
                                                <th>Violator</th> <!-- Updated to Violator -->
                                                <th>Sex</th>
                                                <th>Address</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach($violation->violators as $violator)
                                                <tr>
                                                    <td>{{ $violator->violator }}</td> <!-- Using consolidated violator field -->
                                                    <td>{{ $violator->sex }}</td>
