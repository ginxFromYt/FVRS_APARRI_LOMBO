    ...
    public function store(Request $request)
{
    ...
    // Check if a report with the same contact number and information already exists
    $existingReport = UserReport::where('contact_number', $validatedData['contact_number'])
        ->where('information', $validatedData['information'])
        ->first();

    if ($existingReport) {
        return redirect()->route('report.form')->with('error', 'A report with the same contact number and information has already been submitted.');
    }
